...
type DeviceTrustServiceClient interface {
	...
	// CreateEnrollPairing initiates the mobile device enrollment ceremony for the
	// calling user. The returned EnrollPairing carries the pairing token that the
	// Web UI encodes into a QR code. The mobile app reads the token and exchanges
	// it for an enrollment token through the public Device Trust service.
	//
	// EnrollPairings are one-per-user. The server overwrites metadata.name with
